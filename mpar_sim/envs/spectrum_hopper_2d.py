...
class SpectrumHopper2D(gym.Env):
  ...
  def step(self, action: np.ndarray):
    ...
    comms_spectrum = np.zeros(self.fft_size)
    for interferer in self.interference:
      interferer.step(time=self.time)
      if interferer.is_active:
        interference_stop_freq = interferer.start_freq + interferer.bandwidth
        occupied = np.logical_and(self.freq_axis >= interferer.start_freq,
                                  self.freq_axis <= interference_stop_freq)
        self.spectrogram[0, -1, occupied] = 1

        # Also check occupation for the reward computation
        occupied = np.logical_and(self.fft_freq_axis >= interferer.start_freq,
                                  self.fft_freq_axis <= interference_stop_freq)
        comms_spectrum[occupied] = 1

    # Radar spectrum occupancy
    occupied = np.logical_and(self.fft_freq_axis >= radar_start_freq,
    ...
    collision_penalty = -np.sum(np.logical_and(comms_spectrum,
                                               radar_spectrum)) / np.sum(comms_spectrum)
  ...
  def reset(self, seed: int = None, options: dict = None):
    ...
    for interferer in self.interference:
      interferer.reset()
    self.spectrogram = np.zeros((1, 84, 84), dtype=np.uint8)
    self.time = 0
  ...
  def _render_frame(self) -> Optional[np.ndarray]:
    ...
    pixels = self.spectrogram[0].T*255
