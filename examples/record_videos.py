...
env = make_env(env_id,  radar, max_episode_steps, seed)()
# env = gym.wrappers.RecordVideo(env, video_folder='videos/', video_length=1000)
...
while not np.all(dones):
  frames.append(np.flip(env.render(), axis=1))
...
fig = plt.figure()
plt.xlabel('Azimuth (\N{DEGREE SIGN})')
plt.ylabel('Elevation (\N{DEGREE SIGN})')
ims = []
for frame in frames:
  im = plt.imshow(np.clip(frame, 0, 50), animated=True,
                  cmap='viridis', extent=[-45, 45, -45, 45])
  ims.append([im])

anim = animation.ArtistAnimation(
    fig, ims, interval=50, blit=True, repeat_delay=1000)
anim.save(
    f'videos/raster_spso.mp4', writer='ffmpeg', fps=30)
env.close()
